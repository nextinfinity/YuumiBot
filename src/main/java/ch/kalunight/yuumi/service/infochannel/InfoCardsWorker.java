...
import ch.kalunight.yuumi.model.InfoCard;
...
public class InfoCardsWorker implements Runnable {
    ...
    try {
      if(controlPanel.canTalk()) {
        if(account.summoner == null) {
          account.summoner = Yuumi.getRiotApi()
  ...
  private void generateInfoCard(DTO.LeagueAccount account, DTO.CurrentGameInfo currentGameInfo)
    ...
    if(card != null) {
      ...
      TextChannel infoChannel = Yuumi.getJda().getGuildById(server.serv_guildId).getTextChannelById(infochannel.infochannel_channelid);
