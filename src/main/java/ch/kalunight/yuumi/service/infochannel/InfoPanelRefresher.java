package ch.kalunight.yuumi.service.infochannel;
...
import ch.kalunight.yuumi.model.ComparableMessage;
import ch.kalunight.yuumi.model.config.ServerConfiguration;
import ch.kalunight.yuumi.model.dto.DTO;
import ch.kalunight.yuumi.model.dto.DTO.InfoChannel;
import ch.kalunight.yuumi.model.dto.DTO.InfoPanelMessage;
...
import ch.kalunight.yuumi.model.dto.DTO.ServerStatus;
import ch.kalunight.yuumi.model.dto.GameInfoCardStatus;
import ch.kalunight.yuumi.model.player_data.FullTier;
import ch.kalunight.yuumi.repositories.ConfigRepository;
import ch.kalunight.yuumi.repositories.CurrentGameInfoRepository;
import ch.kalunight.yuumi.repositories.GameInfoCardRepository;
...
import ch.kalunight.yuumi.service.rankchannel.RankedChannelLoLRefresher;
import ch.kalunight.yuumi.translation.LanguageManager;
import ch.kalunight.yuumi.util.InfoPanelRefresherUtil;
import ch.kalunight.yuumi.util.TreatedPlayer;
...
public class InfoPanelRefresher implements Runnable {
  ...
  public InfoPanelRefresher(DTO.Server server) {
    this.server = server;
    guild = Yuumi.getJda().getGuildById(server.serv_guildId);
  }
    ...
    try {
      ...
      try {
        ...
        PermissionOverride permissionOverride = infochannel
            .putPermissionOverride(guild.retrieveMember(Yuumi.getJda().getSelfUser()).complete()).complete();
    ...
    for(DTO.InfoPanelMessage messageToClearReaction : InfoChannelRepository.getInfoPanelMessages(server.serv_guildId)) {
      ...
      if(retrievedMessage != null) {
        for(MessageReaction messageReaction : retrievedMessage.getReactions()) {
          try {
            messageReaction.removeReaction(Yuumi.getJda().getSelfUser()).queue();
          } catch (ErrorResponseException e) {
  ...
  private void cleaningInfoChannel() throws SQLException {
    ...
    List<Message> messagesToCheck = infochannel.getIterableHistory().stream()
        .limit(1000)
        .filter(m-> m.getAuthor().getId().equals(Yuumi.getJda().getSelfUser().getId()))
