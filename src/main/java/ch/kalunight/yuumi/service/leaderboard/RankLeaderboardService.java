...
import ch.kalunight.yuumi.model.leaderboard.dataholder.PlayerRank;
import ch.kalunight.yuumi.model.leaderboard.dataholder.QueueSelected;
	...
	protected void runLeaderboardRefresh(Server server, Guild guild, TextChannel channel, Leaderboard leaderboard,
		...
		for(PlayerRank playerRank : playersRank) {
			...
			if(queueSelected == null) {
				dataList.add(Resources.getTierEmote().get(fullTier.getTier()).getUsableEmote() + " " + fullTier.toString(server.serv_language)
