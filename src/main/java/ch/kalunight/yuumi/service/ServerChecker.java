...
import ch.kalunight.yuumi.service.leaderboard.LeaderboardBaseService;
	...
	public void run() {
		...
		try {
			...
			if(nextDiscordBotListRefresh.isBeforeNow()) {
				...
				if(Yuumi.getBotListApi() != null) {
					// Discord bot list status
					Yuumi.getBotListApi().setStats(Yuumi.getJda().getGuilds().size());
				}
			...
			logger.debug("Server-Executor Queue : {}", ServerData.getServerExecutor().getQueue().size());
			logger.debug("InfoCards-Generator Queue : {}", ServerData.getInfocardsGenerator().getQueue().size());
			logger.debug("Number of cached users: {}", Yuumi.getJda().getUserCache().size());
		}
	}
