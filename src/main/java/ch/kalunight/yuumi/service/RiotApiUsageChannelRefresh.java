...
import ch.kalunight.yuumi.repositories.LeaderboardRepository;
...
public class RiotApiUsageChannelRefresh implements Runnable {
	...
	public void run() {
		...
		try {
			...
			Guild guild = Yuumi.getJda().getGuildById(guildId);
			...
			if(rapiInfoChannel != null) {
				...
				rapiInfoChannel.sendMessage("**Generic Stats**"
						+ "\nTotal number of Servers : " + Yuumi.getJda().getGuilds().size()
				...
				rapiInfoChannel.sendMessage("**Riot Request Stats**"
						+ "\nTotal of requests with Riot api : " + Yuumi.getRiotApi().getTotalRequestCount()
						+ "\nNumber of request for match with RiotAPI : " + Yuumi.getRiotApi().getApiMatchRequestCount()
						+ "\nNumber of request for match with DB : " + (Yuumi.getRiotApi().getAllMatchRequestCount() - Yuumi.getRiotApi().getApiMatchRequestCount())).queue();

				rapiInfoChannel.sendMessage("**Discord Command Stats**"
						+ "\nTotal discord command executed : " + YuumiCommand.getCommandExecuted().get()
						+ "\nTotal discord command done correctly : " + YuumiCommand.getCommandFinishedCorrectly().get()
						+ "\nTotal discord command done with error : " + YuumiCommand.getCommandFinishedWithError().get()).queue();
				...
				for(Platform platform : Platform.values()) {
					long numberOfRequestRemaining = Yuumi.getRiotApi().getApiCallRemainingPerRegion(platform);
				...
				for(Platform platform : Platform.values()) {
					long numberOfRequestRemaining = Yuumi.getRiotApi().getApiCallRemainingPerRegionTFT(platform);
				...
				if(Yuumi.getRiotApi().isApiCallPerPlatformNeedToBeReset()) {
					Yuumi.getRiotApi().resetApiCallPerPlatform();
				}
			}
		}catch(Exception e) {
	...
	private void cleanChannel(TextChannel rapiInfoChannel) {
		...
		try {
			...
			List<Message> onlyMyMessagesToDelete = rapiInfoChannel.getIterableHistory().stream()
					.filter(m -> m.getAuthor().equals(Yuumi.getJda().getSelfUser()))
					.collect(Collectors.toList());
	...
	public static TextChannel getRapiInfoChannel() {
		if(Yuumi.getJda().getGuildById(guildId) != null) {
			return Yuumi.getJda().getGuildById(guildId).getTextChannelById(textChannelId);
		}
		return null;
	}
