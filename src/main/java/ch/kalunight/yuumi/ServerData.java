...
public class ServerData {
	...
	/**
	 * Used by event waiter, define in {@link Yuumi#main(String[])}
	 */
	...
	static {
		...
		SERVER_EXECUTOR.setThreadFactory(new ThreadFactoryBuilder().setNameFormat("Yuumi Server-Executor-Thread %d").build());
		INFOCARDS_GENERATOR.setThreadFactory(new ThreadFactoryBuilder().setNameFormat("Yuumi InfoCards-Generator-Thread %d").build());
		RANKED_MESSAGE_GENERATOR.setThreadFactory(new ThreadFactoryBuilder().setNameFormat("Yuumi Ranked-Message-Generator-Thread %d").build());
		RESPONSE_WAITER.setThreadFactory(new ThreadFactoryBuilder().setNameFormat("Yuumi Response-Waiter-Thread %d").build());
		LEADERBOARD_EXECUTOR.setThreadFactory(new ThreadFactoryBuilder().setNameFormat("Yuumi Leaderboard-Refresher-Thread %d").build());
		COMMANDS_EXECUTOR.setThreadFactory(new ThreadFactoryBuilder().setNameFormat("Yuumi Command-Executor-Thread %d").build());
		...
		for(Platform platform : Platform.values()) {
			...
			String nameOfThread = String.format("Yuumi Infochannel-Helper-%s-Worker", platform.getName().toUpperCase());
			...
			nameOfThread = String.format("Yuumi Match-%s-Worker", platform.getName().toUpperCase());
			executor.setThreadFactory(new ThreadFactoryBuilder().setNameFormat(nameOfThread + " %d").build());
			MATCH_THREAD_EXECUTORS.put(platform, executor);

			executor = new ThreadPoolExecutor(NBR_PROC, NBR_PROC, 3, TimeUnit.MINUTES, new LinkedBlockingQueue<Runnable>());
			nameOfThread = String.format("Yuumi Player-Data-%s-Worker", platform.getName().toUpperCase());
			executor.setThreadFactory(new ThreadFactoryBuilder().setNameFormat(nameOfThread + " %d").build());
