				...
				try {
					if(emoteUsed.getName().equals("✅")) {
						...
						if(infochannelDb != null) {
							...
							PermissionOverride permissionYuumiPlayer = infochannel.putPermissionOverride(role).complete();
							permissionYuumiPlayer.getManager().grant(Permission.MESSAGE_READ, Permission.MESSAGE_HISTORY).complete();

							PermissionOverride permissionYuumi = infochannel
									.putPermissionOverride(guild.retrieveMember(Yuumi.getJda().getSelfUser()).complete()).complete();
							permissionYuumi.getManager().grant(Permission.MESSAGE_READ, Permission.MESSAGE_HISTORY).complete();
						...
						if(rankChannelDb != null) {
							...
							PermissionOverride permissionYuumiPlayer = rankChannel.putPermissionOverride(role).complete();
							permissionYuumiPlayer.getManager().grant(Permission.MESSAGE_READ, Permission.MESSAGE_HISTORY).complete();

							PermissionOverride permissionYuumi = rankChannel
									.putPermissionOverride(guild.getMember(Yuumi.getJda().getSelfUser())).complete();
							permissionYuumi.getManager().grant(Permission.MESSAGE_READ, Permission.MESSAGE_HISTORY).complete();
