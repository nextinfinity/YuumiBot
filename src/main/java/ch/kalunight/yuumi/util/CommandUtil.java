...
public class CommandUtil {
	...
	public static void sendTypingInFunctionOfChannelType(CommandEvent event) {
	...
	public static MessageChannel getFullSpeakableChannel(Guild guild) {
		...
		Member yuumiMember = guild.retrieveMember((Yuumi.getJda().getSelfUser())).complete();

		for(TextChannel textChannel : guild.getTextChannels()) {
			if(textChannel.canTalk()) {
				Set<Permission> permissions = yuumiMember.getPermissions(textChannel);
	...
	public static void giveRolePermission(Guild guild, TextChannel channel,
		...
		try {
			...
			channel.putPermissionOverride(guild.retrieveMember(Yuumi.getJda().getSelfUser()).complete())
					.grant(Permission.MESSAGE_READ, Permission.MESSAGE_HISTORY).queue();

			channel.putPermissionOverride(serverConfiguration.getYuumiRoleOption().getRole())
					.grant(Permission.MESSAGE_READ, Permission.MESSAGE_HISTORY).queue();
		}catch(InsufficientPermissionException e) {
			logger.info("Missing permission to apply Yuumi role option rule, nothing bad");
		}
	}
			...
			public void accept(CommandEvent event) {
				...
				if(event.getChannelType() == ChannelType.TEXT) {
					try {
						...
						Yuumi.logger.error("Error when doing help command message !", e);
