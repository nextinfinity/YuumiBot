...
import ch.kalunight.yuumi.model.KDAReceiver;
...
import ch.kalunight.yuumi.service.match.MatchKDAReceiverWorker;
import ch.kalunight.yuumi.service.match.MatchReceiverWorker;
import ch.kalunight.yuumi.service.match.MatchWinrateReceiverWorker;
import ch.kalunight.yuumi.translation.LanguageManager;
import ch.kalunight.yuumi.util.NameConversion;
...
public class RiotRequest {
	...
	public static FullTier getSoloqRank(String summonerId, Platform region) {
		...
		try {
			listLeague = Yuumi.getRiotApi().getLeagueEntriesBySummonerIdWithRateLimit(region, summonerId);
	...
	public static String getWinrateLastMonthWithGivenChampion(String summonerId, Platform region,
		...
		Summoner summoner;
		try {
			summoner = Yuumi.getRiotApi().getSummonerWithRateLimit(region, summonerId);
		} catch(RiotApiException e) {
	...
	private static List<MatchReference> getMatchHistoryOfLastMonthWithTheGivenChampion(Platform region, int championKey, Summoner summoner)
		...
		do {
			...
			try {
				matchList = Yuumi.getRiotApi().getMatchListByAccountIdWithRateLimit(region, summoner.getAccountId(), championToFilter, null, null,
						-1, -1, startIndex, -1);
	...
	public static String getMasterysScore(String summonerId, int championId, Platform platform) {
		...
		try {
			...
			masteryString.append(Resources.getMasteryEmote().get(masteryLevel).getEmote().getAsMention());
