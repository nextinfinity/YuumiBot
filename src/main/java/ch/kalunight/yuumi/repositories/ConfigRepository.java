...
public class ConfigRepository {
	...
	public static ServerConfiguration getServerConfiguration(long guildId) throws SQLException {
		...
		try (Connection conn = RepoResources.getConnection();
			...
			serverConfig.setYuumiRoleOption(roleOption);
	...
	private static RoleOption getRoleOption(ResultSet result, long guildId) throws SQLException {
		...
		try {
			...
			if(role == null) {
				...
				TextChannel infoChannel = Yuumi.getJda().getTextChannelById(infoChannelDb.infochannel_channelid);
				...
				TextChannel rankChannel = Yuumi.getJda().getTextChannelById(rankChannelDb.rhChannel_channelId);
	...
	private static CleanChannelOption getCleanChannelOption(long guildId, ResultSet result) throws SQLException {
		...
		if(!cleanChannelOption.getCleanChannelOption().equals(CleanChannelOptionInfo.DISABLE)) {
			...
			try {
				TextChannel cleanChannel = Yuumi.getJda().getGuildById(guildId).getTextChannelById(cleanChannelId);
