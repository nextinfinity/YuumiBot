...
public class ShutDownCommand extends YuumiCommand {
	...
	protected void executeCommand(CommandEvent event) {
		...
		for(Object eventListener : Yuumi.getEventlistenerlist()) {
			Yuumi.getJda().removeEventListener(eventListener);
		}

		Yuumi.getJda().getPresence().setPresence(OnlineStatus.DO_NOT_DISTURB, Activity.of(ActivityType.DEFAULT, "Shuting down ..."));
		...
		while(!Yuumi.getJda().getStatus().equals(Status.SHUTDOWN)) {
			try {
				Thread.sleep(100);
			} catch(InterruptedException e) {
