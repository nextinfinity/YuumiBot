...
public class CreatePlayerCommand extends YuumiCommand {
	...
	public static void updateLastRank(LeagueAccount leagueAccount) throws SQLException {
		...
		try {
			Set<LeagueEntry> leagueEntries = Yuumi.getRiotApi().
			...
			Yuumi.logger.info("Fail to refresh LoL last rank while creating a leagueAccount, will be done at the next game.");
		}
		...
		try {
			...
			Yuumi.logger.info("Fail to refresh TFT last rank while creating a leagueAccount, will be done at the next game.");
		}
	}
